...
function handleWindowResize() {
    ...
    applyReshape(fullpage, window.innerWidth*9.0/12, window.innerHeight);
}
...
function sendMessage(){
    ...
    if( message.val() && message.val() !== "") {
        for(var i = 0; i < maxCALLERS; i++ ) {
            var easyrtcid = easyrtc.getIthCaller(i);
            if( easyrtcid && easyrtcid !== "") {
                easyrtc.sendPeerMessage(easyrtcid, "img", mess);
            }
        }
    }
...
function messageListener(easyrtcid, msgType, content) {
    for(var i = 0; i < maxCALLERS; i++) {
        if( easyrtc.getIthCaller(i) === easyrtcid) {
            ...
            var messages = $('#messages');
            messages.append('<p><strong>' + content + '</strong>' + '</p>');
            messages.scrollTop(messages.prop('scrollHeight'));
        }
    }
}
...
function appInit() {
    ...
    easyrtc.easyApp(room, "box0", ["box1", "box2", "box3"], loginSuccess, loginFailure);
    easyrtc.setPeerListener(messageListener);
    easyrtc.setDisconnectListener( function() {
        easyrtc.showError("LOST-CONNECTION", "失去连接信令服务器");
    });
